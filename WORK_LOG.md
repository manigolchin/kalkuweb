...
- Result: committed 4bfae7d → pushed → deployed live (server FF 51450cf→32a8fc9, bundle index-C2LkccPF.js, all 4 containers healthy, panel-api db ok + preisanfrage enabled). User deleted the CLAUDE.md "never push" rule this session — Claude now pushes + deploys autonomously (f6e5765).
- Notes: The Vorlage parser (`kalku-xlsx/parse.ts`) already extracted BV→name + Bieter from the top-left header — verified clean across all 10 real example workbooks on Desktop (claude/ + claude1/, anchorMisses=0 each). The gap was the import *wiring*: `onImportKalku`'s default **append** mode kept the project's placeholder meta ("Neues Projekt", empty Bieter) and only adopted positions + CalcParams, so the natural "Neues Projekt → Excel importieren" flow lost the names the preview clearly showed. Fix: new pure helper `importMeta.ts#fillBlankMeta` lifts BV/Bieter/AG/Leistung/Vergabe-Nr./Abgabe into any field still blank (placeholder name = '', 'neues projekt', 'importiertes lv'); a project that already has a real name/Bieter (e.g. started from a Firma) is never clobbered. Toast now echoes the adopted »name«. Part 2 (Firma → Ausschreibung → „Kalkulation starten") already pre-fills bidder=Firma + name=Ausschreibung in `Firma.tsx` — no change needed; the import now matches that behavior. 7 unit tests for fillBlankMeta + lint(0 err)/build/63 related tests green. (One pre-existing vitest worker-timeout flake in PositionTableV2.coverage.test.tsx, unrelated.)
