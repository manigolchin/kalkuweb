...
function ReadingPane({ email, companyName, onBack }: { email: PosteingangEmail; companyName: string; onBack: () => void }) {
  ...
  // Reset scroll + close any open reply draft whenever the selected email changes.
  useEffect(() => {
  ...
  async function sendReply() {
    ...
    setSending(true);
    try {
      const res = await api.posteingang.reply(email.id, text);
      toast.success(`Antwort gesendet an ${res.to ?? email.fromEmail ?? 'Empfänger'}`);
      setReplyOpen(false);
      setReplyText('');
    } catch (e) {
      const code = e instanceof ApiError ? (e.body as { error?: string } | null)?.error : undefined;
      if (code === 'reply_unavailable')
        toast.error('Antworten ist serverseitig noch nicht aktiv (preisanfrage-Update nötig).');
      else if (code === 'send_failed') toast.error('Senden fehlgeschlagen (SMTP).');
      else if (code === 'cannot_reply') toast.error('Auf diese E-Mail kann nicht geantwortet werden.');
      else toast.error('Antwort konnte nicht gesendet werden.');
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <>
      ...
      {/* actions / reply composer */}
      {replyOpen ? (
        <div className="px-4 sm:px-6 py-3 border-t border-slate-200 dark:border-slate-800 space-y-2">
          <div className="text-xs text-slate-500 dark:text-slate-400">
            An: <span className="font-medium text-slate-700 dark:text-slate-300">{email.fromName || email.fromEmail}</span>
            {email.fromEmail && email.fromName && <span> &lt;{email.fromEmail}&gt;</span>}
            <span className="mx-1.5">·</span>
            Betreff: <span className="text-slate-600 dark:text-slate-400">AW: {email.subject || '(kein Betreff)'}</span>
          </div>
          <textarea
            value={replyText}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3.5 h-9 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-50"
            >
        ...
        <footer className="px-4 sm:px-6 py-2.5 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2">
          <button
            ...
            disabled={!canReply}
            title={canReply ? 'Im Panel antworten' : 'Keine Absenderadresse'}
